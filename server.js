...
// MongoDB Connection
const mongoUri =
  process.env.MONGO_URI || "mongodb://localhost:27017/alumifyxDB";
mongoose
  .connect(mongoUri, {
  ...
  async (req, res) => {
    try {
      ...
      if (phone && !/^\+?\d{10,15}$/.test(phone)) {
        return res
          .status(400)
          .json({ error: "Invalid phone number (10-15 digits)" });
      }
...
app.post("/api/mentorship", isAuthenticated, async (req, res) => {
  try {
    ...
    if (existingMentorship) {
      return res
        .status(400)
        .json({ error: "You have already purchased this mentorship" });
    }
...
app.get("/api/mentorships", isAuthenticated, async (req, res) => {
  try {
    const mentorships = await Mentorship.find({
      userId: req.session.userId,
    }).populate("mentorId", "name skill photo bio experience price");
...
app.get("/api/library", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Server error while fetching library resources" });
  }
});
...
app.get("/api/recorded-classes", async (req, res) => {
  try {
    const classes = await RecordedClass.find().populate(
      "skillId",
      "name description"
    );
    res.json(classes);
    ...
    res
      .status(500)
      .json({ error: "Server error while fetching recorded classes" });
  }
});
...
app.get("/api/live-classes", async (req, res) => {
  try {
    const classes = await LiveClass.find().populate(
      "skillId",
      "name description"
    );
    res.json(classes);
  ...
  try {
    ...
    const resources = [
      ...
      },
      {
        title: "Binary Trees",
        type: "Video",
        url: "https://example.com/binary-trees.mp4",
        chapterId: null,
      },
    ];
    ...
    const recordedClasses = [
      ...
      },
      {
        title: "HTML & CSS",
        skillId: null,
        url: "https://example.com/html-css.mp4",
        duration: "1.5 hours",
      },
    ];
